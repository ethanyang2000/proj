...
from utils import myRecord, get_obj_pos, get_pos_and_rot, l2_dis, euler_yaw, all_moving
...
class myBot(AddOn):
    ...
    def navigate_to(self, pos):
        ...
        if pos[0] > curr_pos[0]:
            ...
            theta = 0 if pos[2] > curr_pos[2] else 180
...
h = [myBot(id=c.get_unique_id(), position=posi[j]) for j in range(AGENTS_)]
...
path = EXAMPLE_CONTROLLER_OUTPUT_PATH.joinpath("new")
    ...
    if r_id == "tran":
        ...
        for j in range(transforms.get_num()):
            if transforms.get_id(j) == obj_id:
                ...
                ic(obj_pos)
h[0].navigate_to(obj_pos)
h[1].navigate_to(obj_pos)
while all_moving(h):
    c.communicate([])
h[0].pick_up(obj_id)
h[1].move_by(3)
